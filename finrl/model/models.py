...
class DRLAgent:
    ...
    def DRL_prediction(model, environment):
        ...
        state_list = []
        state_list.append(test_obs)

        """make a prediction"""
        test_env.reset()
        df_value_pro = pd.DataFrame({
            'value': [],
            'prob': []
        })
        ...
        for i in range(len(environment.df.index.unique())):
            new_state_list = []
            while state_list:
                _last_obs = state_list.pop()

                action, _states = model.predict(_last_obs)
                with th.no_grad():
                    # Convert to pytorch tensor
                    obs_tensor = th.as_tensor(_last_obs).to(device)
                    act_tensor = th.as_tensor(action).to(device)

                #     actions1, values1, log_probs1 = model.policy.forward(
                #         obs_tensor) #,deterministic=True
                #     actions2 = model.policy._predict(obs_tensor) #,deterministic=True

                '''    value, log_prob,entropy= model.policy.evaluate_actions(obs_tensor,act_tensor)'''

                #     values3, log_probs3, entropy2 = model.policy.evaluate_actions(obs_tensor, actions2)
                #
                # actions1 = actions1.cpu().numpy()
                # actions2 = actions2.cpu().numpy()
                # print(f"\nactions1:{actions1}/values1:{values1}/log_probs1:{log_probs1}")
                # print(f"actions2:{actions2}")
                # print(f"values2:{values2}/log_probs2:{log_probs2}")
                # print(f"values3:{values3}/log_probs3:{log_probs3}")
                ...
                df_value_pro.loc[len(df_value_pro)] = [value, prob]'''

                new_obs, rewards, action, dones, info = test_env.steps(
                    action, _last_obs[0])


                if action == 1:
                    new_state_list.append(new_obs)
                elif action == 2:
                    new_state_list.append(new_obs)
                    if int(_last_obs[0][0]) == -1:
                        zero_hold_obs, _, _, _, _ = test_env.steps(
                            [1], [-1] + [0] * len(_last_obs[0] - 1))
                        new_state_list.append(zero_hold_obs)

                # if i == (len(environment.df.index.unique()) - 2):

                if dones[0]:
                    print("hit end!")
                    return trade_cache[0],trade_state_memory[0],empty_state_memory[0]
                else:
                    trade_cache = test_env.env_method(
                        method_name="save_trade_cache")
                    trade_state_memory = test_env.env_method(
                        method_name="save_trade_state_memory")
                    empty_state_memory = test_env.env_method(
                        method_name="save_empty_state_memory")
                    # asset = test_env.env_method(
                    #     method_name="save_final_asset")

            state_list = new_state_list

        return None  # account_memory[0], actions_memory[0]
    ...
    def train_model(self, model, tb_log_name, total_timesteps=5000, save_path='/home/cyc/RL_STOCK/',per_save_timesteps= 5,early_stopping_patience=10):
        model = model.learn(
    ...
    def run_ensemble_strategy(
        ...
        for i in range(self.rebalance_window + self.validation_window,
            ...
            if historical_turbulence_mean > insample_turbulence_threshold:
                ...
                # meaning the current turbulence can't exceed the 90% quantile
                # of insample turbulence data
                turbulence_threshold = insample_turbulence_threshold
            else:
                # if the mean of the historical data is less than the 90% quantile of insample turbulence data
                # then we tune up the turbulence_threshold, meaning we lower
                # the risk
            ...
            print("======Best Model Retraining from: ",
                  self.train_period[0],
                  "to ",
