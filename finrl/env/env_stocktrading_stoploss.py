...
class StockTradingEnvStopLoss(gym.Env):
    ...
    def get_reward(self):
        if self.current_step == 0:
            ...
            if self.current_step > 1:
                ...
                stop_loss_penalty = -1 * \
                    np.dot(np.array(prev_holdings), neg_closing_diff_avg_buy)
            else:
                stop_loss_penalty = 0
            low_profit_penalty = -1 * \
                np.dot(np.array(holdings), neg_profit_sell_diff_avg_buy)
            ...
            reward = ((total_assets - total_penalty +
                       additional_reward) / self.initial_amount) - 1
    ...
    def step(self, actions):
        ...
        if self.date_index == len(self.dates) - 1:
            ...
            # buy/sell only if the price is > 0 (no missing data in this
            # particular date)
            actions = np.where(closings > 0, actions, 0)
            if self.turbulence_threshold is not None:
                # if turbulence goes over threshold, just clear out all
                # positions
            ...
            if self.discrete_actions:
                ...
                # round down actions to the nearest multiplies of
                # shares_increment
                actions = np.where(
                    actions >= 0,
                    (actions //
            ...
            self.profit_sell_diff_avg_buy = np.where(
                profit_sell == 1, closings - (self.min_profit_penalty * self.avg_buy_price), 0)
